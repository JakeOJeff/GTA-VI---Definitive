...
function button:initialize(text, code, x, heightS, rx, ry, textColor, font, color, hover, click)
...
function button:update()
	...
	if x < self.x + self.font:getWidth(self.text) + 20 and x > self.x and y < self.height + self.font:getHeight(self.text) + 20 and y > self.height then 
...
function button:draw()
	...
	love.graphics.rectangle("fill", self.x, self.height, self.font:getWidth(self.text) + 20, self.font:getHeight(self.text) + 20, self.rx, self.ry)

	love.graphics.setColor(self.textColor)
	love.graphics.print(self.text, self.x + 10, self.height + 10)
