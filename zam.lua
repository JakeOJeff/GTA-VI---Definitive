...
function zam_window:load()
	...
	window.translate.y = -char.y + love.graphics.getHeight() - (love.graphics.getHeight()/6)
end
...
function zam_window:update_translation ()
	...
	if love.mouse.isDown(3) then
		...
		if char.x > love.graphics.getWidth()/2 then
			window.translate.x = -char.x  + love.graphics.getWidth()/4 
			window.translate.y = -char.y + love.graphics.getHeight() - (love.graphics.getHeight()/6)
	
		else
			window.translate.x = 0 
			window.translate.x = 0
			window.delta.x = (window.translate.x)/window.zoom
			window.delta.y = (window.translate.y)/window.zoom
		end
	end
end
